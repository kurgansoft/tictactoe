...
The resulting jar file will be in the out/TicTacToe folder. 
...
```
java -DhostAddress="192.168.0.5" -Dport=8765 -jar tictactoe.jar
```
...
1) Building the UI: 
   ...
   * Issue this command: ```mill TicTacToe.ui.fastOpt```
   * In case you want the optimized version: ```mill TicTacToe.ui.fullOpt```
   
2) Building the backend: ```mill TicTacToe.backend.compile```
   * Before you build the backend make sure that the jsLocation in the [CustomLauncher](TicTacToe/backend/src/base/CustomLauncher.scala) object 
